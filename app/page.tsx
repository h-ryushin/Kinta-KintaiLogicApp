...
function AttendanceContent() {
  ...
  const startListening = (staffId: string) => {
      // ブラウザの標準APIか、WebKit(Safari/Chrome)用APIを取得
      const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;

      if (!SpeechRecognition) {
        alert("お使いのブラウザは音声入力に対応していません。iPhoneならSafari、PCならGoogle Chromeをお試しください。");
        return;
      }
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-blue-50/50 rounded-3xl p-6 border border-blue-100/50 space-y-3">
            ...
            <p className="text-[11px] font-bold text-slate-500 leading-relaxed">
              ...
              <br />
              <span className="text-blue-500 text-[10px]">音声入力の精度は完全ではないので目視の確認してください！<br></br>あとセキュリティの問題で音声入力ができない機種や検索エンジンがあります。。</span>
            </p>
          </div>
